...
def grid_sample(input, grid, canvas=None):
    """
    In case the ctrl points are located outside of the cropped image,
    use 'canvas' to
    """
...
def compute_partial_repr(input_points, control_points):
    ...
    # pairwise_dist = torch.sum(pairwise_diff ** 2, dim = 2) # square of
    # distance
...
class TPSSpatialTransformer(nn.Module):
    ...
    def forward(self, input, source_control_points):
        ...
        source_coordinate = torch.matmul(
            self.target_coordinate_repr, mapping_matrix)

        grid = source_coordinate.view(-1,
                                      self.target_height,
                                      self.target_width,
                                      2)
        # the source_control_points may be out of [0, 1].
        grid = torch.clamp(grid, 0, 1)
        # the input to grid_sample is normalized [-1, 1], but what we get is
        # [0, 1]
        grid = 2.0 * grid - 1.0
