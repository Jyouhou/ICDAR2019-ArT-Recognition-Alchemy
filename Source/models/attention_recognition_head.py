...
from .recognition_fan import FANet
from .recognition_subnet import RecSubNet
...
class AttentionRecognitionHead(nn.Module):
  ...
  def __init__(self, num_classes, in_planes, sDim, attDim, max_len_labels):
    ...
    self.device = torch.device("cuda" if global_args.cuda else "cpu")

    self.encoder = RecSubNet(in_planes)
    self.decoder = DecoderUnit(sDim=sDim, xDim=self.encoder.out_planes, yDim=num_classes, attDim=attDim)
  ...
  def forward(self, x):
    ...
    for i in range(max(lengths)):
      ...
      output, state = self.decoder(encoder_feats, state, y_prev)
  ...
  def beam_search(self, x, beam_width, eos):
    ...
    encoder_feats = self.encoder(x)
    encoder_feats = encoder_feats.contiguous()
    l, d = encoder_feats.size(1), encoder_feats.size(2)
    # inflated_encoder_feats = _inflate(encoder_feats, beam_width, 0) # ABC --> AABBCC -/-> ABCABC
    inflated_encoder_feats = encoder_feats.unsqueeze(1).permute((1,0,2,3)).repeat((beam_width,1,1,1)).permute((1,0,2,3)).contiguous().view(-1, l, d)
