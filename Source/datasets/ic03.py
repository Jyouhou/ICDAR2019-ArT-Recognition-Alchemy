...
class IC03(data.Dataset):
    ...
    def __init__(
        ...
        assert self.lexicon_type in ['0', '50', 'full']
        ...
        with open(os.path.join(lexicon_root_dir, 'ic03.l50'), 'r') as f:
            lines = f.readlines()
            for line in lines:
                line = line.strip()
                image_path = line.split(' ')[0]
                image_path = str(int(image_path.split(
                    '/')[1].split('.')[0].split('_')[1]) - 1)
        ...
        with open(os.path.join(lexicon_root_dir, 'ic03.full'), 'r') as f:
            lines = f.readlines()
            lexiconfull = [line.strip().lower() for line in lines]
        for k, v in self.lexicons50.items():
            self.lexiconsfull[k] = lexiconfull
    ...
    def _get_single_item(self, index):
        ...
        nid, word, file_name = self.nid_labels[index]
        ...
        if global_args.REC_SQUARE == 0:
            ...
            img, ratio = TextSquare(img.astype(np.float64), global_args.REC_SQUARE)
            img = Rotation(img)
        ...
        if global_args.evaluate_with_lexicon:
            return (img, label, len(label_list), maps, ctrl_points, file_name)
        else:
