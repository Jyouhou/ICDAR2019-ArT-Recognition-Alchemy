...
class IIIT5K(data.Dataset):
    ...
    def __init__(
        ...
        assert self.lexicon_type in ['0', '50', '1k']
        ...
        with open(os.path.join(lexicon_root_dir, 'iiit5k.l50'), 'r') as f:
            lines = f.readlines()
            for line in lines:
                line = line.strip()
        ...
        with open(os.path.join(lexicon_root_dir, 'iiit5k.1k'), 'r') as f:
            lines = f.readlines()
            for line in lines:
                line = line.strip()
                image_path = line.split(' ')[0].split('/')[1]
                lexicon1k = line.split(' ')[3].split(',')
                lexicon1k = [w.lower() for w in lexicon1k]
                self.lexicons1k[image_path] = lexicon1k
